...
export function CategorySummary({ items, names, budgets, currency, periodLabel = 'month', selectedCategoryId = '', onSelect, className = '' }: {
  items: CategoryItem[];
  ...
  const relevantBudgets = selectedCategoryId ? budgets.filter((budget) => budget.categoryId === selectedCategoryId) : budgets;
  ...
  return (
    <section className={`overflow-hidden rounded-3xl border border-neutral-200/80 bg-white shadow-sm ${className}`}>
      ...
      {sorted.length === 0 ? <p className="px-4 py-5 text-center text-sm text-neutral-400">No category activity this {periodLabel}.</p> : <div>
        <div className="grid grid-cols-[minmax(0,1.4fr)_minmax(0,1fr)_minmax(0,1fr)_minmax(0,1.25fr)] gap-2 bg-neutral-50/70 px-3 py-2 text-[9px] font-medium uppercase tracking-wide text-neutral-400 sm:grid-cols-[1.6fr_1fr_1fr_1.4fr] sm:px-4 sm:py-2.5 sm:text-xs"><span>Category</span><span className="text-right"><span className="sm:hidden">In</span><span className="hidden sm:inline">Debit</span></span><span className="text-right"><span className="sm:hidden">Out</span><span className="hidden sm:inline">Credit</span></span><span className="text-right">Budget</span></div>
        <div className="divide-y divide-neutral-100">{sorted.map((row) => {
          ...
          return <Link key={row.categoryId} to={`/transactions?category=${row.categoryId}`} className={rowClass}>{cells}</Link>;
        })}</div>
      </div>}
    </section>
  );
